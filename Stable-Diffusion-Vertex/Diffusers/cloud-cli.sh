...
gcloud ai custom-jobs create \
  ...
  --config=vertex-config-nfs.yaml \
  --args="--method=diffuser_dreambooth,--model_name=runwayml/stable-diffusion-v1-5,--input_storage=/gcs/sd_lsj/dog_image,--output_storage=/gcs/sd_lsj/diffusers_db_output,--prompt=a photo of sks dog,--class_prompt=a photo of dog,--num_class_images=50,--lr=1e-4,--use_8bit=True,--max_train_steps=100,--text_encoder=True,--set_grads_to_none=True" \
  --command="python3,train.py"
